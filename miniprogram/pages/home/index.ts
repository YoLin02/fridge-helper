...
import { getLatestWeeklyPlan } from "../../services/plan.service";
import { getRecipeRecommendations } from "../../services/recipe.service";
import type { RecipeRecommendation } from "../../models/recipe";
import type { WeeklyPlan } from "../../models/plan";
...
Page({
  data: {
    ...
    recipeRecommendations: [] as RecipeRecommendation[],
    latestPlan: null as WeeklyPlan | null
  },
  async onShow() {
    try {
  ...
  handleQuickAction(event: WechatMiniprogram.CustomEvent<{ title: string }>) {
  ...
  goRecipeDetail(event: WechatMiniprogram.TouchEvent<{ id: string }>) {
    wx.navigateTo({
