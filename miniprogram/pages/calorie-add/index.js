...
Page({
    ...
    async loadInventoryOptions() {
        try {
            const inventoryOptions = await (0, inventory_service_1.listInventoryBatchOptions)();
            this.setData({
                inventoryOptions
            });
        }
    ...
    async selectInventoryItem(event) {
    ...
    async handleParse() {
        ...
        try {
            const draft = await (0, calorie_service_1.parseCalorieDraft)({
                date: this.data.date,
    ...
    async handleSave() {
        ...
        try {
            if (this.data.mode === "exercise") {
                await (0, calorie_service_1.addExerciseLog)({
                    date: this.data.date,
                ...
                await (0, calorie_service_1.addMealLog)({
                    date: this.data.date,
                    mealType: this.data.mealType,
                ...
                if (this.data.selectedInventory.length > 0) {
                    ...
                    if (result.confirm) {
                        await (0, inventory_service_1.deductInventoryBatches)({
